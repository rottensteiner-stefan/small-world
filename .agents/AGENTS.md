...
- **Terminal Commands:** You are explicitly allowed to run read-only shell commands (like `grep`, `tail`, `cat`, `ls`, `find`, `git log`, `git diff`), image manipulation tools (`sips`), Git operations (`git checkout`, `git add`, `git commit`, `git push`, `git log`, `git branch`), as well as safe project scripts (`npm run lint`, `npm run build`, `npm run test`) in the terminal WITHOUT asking for permission.

...
Instead of using the DOM's `window.dispatchEvent` (which incurs heavy garbage collection overhead and is not strictly typed), you should use the engine's built-in `events` property to broadcast game events.

```typescript
// Inside a Behavior or Controller
this._options.events?.dispatchEvent("player-damage", { amount: 15, source: "lava" });
```

## Listening to Events

Your UI components (e.g., a HUD) or other decoupled systems can simply accept the `Events` interface and listen for specific strings.
...
export class MyHud {
  ...
  private _bindEvents(): void {
    this._events?.addEventListener("player-damage", (e: Record<string, unknown>) => {

...
export class PrologueScene extends AbstractShowcase {
  ...
  protected override async setupScene(): Promise<void> {
    ...
    // 1. Perspective Camera for 2.5D Theatrical Cutaway
    const aspect = window.innerWidth / window.innerHeight;
    this.camera.projection = new PerspectiveProjection({
      fov: MathUtils.degToRad(55),
      aspect,
      near: 0.1,
      far: 100,
    });
    ...
    // 2. Setup Post Processing (Graphic Novel / Outline + Bloom + Noir ToneMapping)
    ...
    // 3. Lighting: Moody Dark Bunker with Door Spot
    ...
    // 4. Build 3D Bunker Koje (2.5D Theatrical Cutaway Stage)
  ...
  private _buildBunkerRoom(): void {
    ...
    const woodMat = new StandardMaterial({
      color: new Color(0.28, 0.19, 0.12),
      roughness: 0.88,
      metallic: 0.05,
    });

    const brassMat = new StandardMaterial({
      color: new Color(0.85, 0.65, 0.25),
      roughness: 0.35,
      metallic: 0.85,
    });

    const glassMat = new GlassMaterial({
    ...
    // 🛏️ Bunk Bed (Stockbett) on Right Wall
    ...
    // ☕ Wooden Shelf & Antique Brass Coffee Grinder (František's Relic)
    ...
    // 📟 The AZS Handheld Terminal (Amts-Terminal 2100)
    ...
    const screenGlass = new Object3D("ScreenGlass");
    screenGlass.geometry = cubeGeo;
    screenGlass.material = glassMat;
    screenGlass.scale.set(0.31, 0.27, 0.01);
    screenGlass.position.set(0, 0.08, 0.065);
    terminal.add(screenGlass);
  ...
  protected override update(deltaTime: number): void {
    ...
    // 1. Smooth Camera Kinematics (Interpolate to target position and target lookAt)
    ...
    // 2. Door Animation (Smooth Hinge Rotation)
  ...
  private _updateTimeline(time: number): void {
    ...
    if (time >= 0 && time < 4.0 && this._currentPhase === 0) {
      ...
      this._targetDoorAngle = -Math.PI / 4.5; // Door cracks open

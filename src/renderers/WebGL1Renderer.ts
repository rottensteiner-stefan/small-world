...
export class WebGL1Renderer implements IRenderer {
  ...
  private getWebGLTexture(tex: Texture): WebGLTexture {
    ...
    if (!glTex) {
      ...
      this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_WRAP_S, this.gl.REPEAT); this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_WRAP_T, this.gl.REPEAT);
  ...
  public render(scene: Scene, vp: Float32Array, camPos: Vector3D = new Vector3D()) {
    ...
    const drawNormal = (o: Object3D) => {
      ...
      if (mat.type === MaterialType.LAMBERT) {
        ...
        shininess = pMat.shininess || 32; specCol = pMat.specularColor ? pMat.specularColor.toArray() : [0, 0, 0, 0];
        if (pMat.diffuseMap) {

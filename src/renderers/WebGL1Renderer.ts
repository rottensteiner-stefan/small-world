...
export class WebGL1Renderer implements IRenderer {
  ...
  public async initialize(canvas: HTMLCanvasElement) {
    this.gl = (canvas.getContext("webgl", { antialias: true }) || canvas.getContext("experimental-webgl")) as WebGLRenderingContext;
    ...
    this.skyProg = createShader(`attribute vec3 a_position; uniform mat4 u_vp; uniform mat4 u_model; varying vec3 v_uvw; void main() { v_uvw = a_position; gl_Position = u_vp * u_model * vec4(a_position, 1.0); }`, `precision highp float; varying vec3 v_uvw; uniform samplerCube u_skybox; void main() { gl_FragColor = textureCube(u_skybox, v_uvw); }`);
  ...
  private getWebGLTexture(tex: Texture): WebGLTexture {
    ...
    if (!glTex) {
      ...
      this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_WRAP_S, this.gl.CLAMP_TO_EDGE); this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_WRAP_T, this.gl.CLAMP_TO_EDGE);
  ...
  private getWebGLCubeTexture(tex: CubeTexture): WebGLTexture {
    ...
    if (!glTex) {
      ...
      for (let i = 0; i < 6; i++) { this.gl.texImage2D(this.gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, this.gl.RGBA, this.gl.RGBA, this.gl.UNSIGNED_BYTE, tex.images[i] as HTMLImageElement); }
      ...
      this.gl.texParameteri(this.gl.TEXTURE_CUBE_MAP, this.gl.TEXTURE_WRAP_S, this.gl.CLAMP_TO_EDGE); this.gl.texParameteri(this.gl.TEXTURE_CUBE_MAP, this.gl.TEXTURE_WRAP_T, this.gl.CLAMP_TO_EDGE);
  ...
  public render(scene: Scene, vp: Float32Array, camPos: Vector3D = new Vector3D()) {
    ...
    const drawNormal = (o: Object3D) => {
      ...
      if (o.geometry && o.material && matType !== SkyboxMaterial.type) {
        ...
        if (matType === LambertMaterial.type) { shininess = 0.0; }
        else if (matType === PhongMaterial.type) {
          const mat = o.material as PhongMaterial; shininess = mat.shininess || 32; specCol = mat.specularColor ? mat.specularColor.toArray() : [0, 0, 0, 0];
  ...
  public setSize(w: number, h: number) { const d = devicePixelRatio; this.gl.canvas.width = w * d; this.gl.canvas.height = h * d; this.gl.viewport(0, 0, this.gl.canvas.width, this.gl.canvas.height); }
}
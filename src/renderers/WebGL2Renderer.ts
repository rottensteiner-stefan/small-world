...
interface ShaderLocs {
  pos: number; norm: number; vp: WebGLUniformLocation | null; model: WebGLUniformLocation | null;
...
export class WebGL2Renderer implements IRenderer {
  ...
  public render(scene: Scene, vp: Float32Array, camPos: Vector3D = new Vector3D()) {
    ...
    const pLights: PointLight[] = []; const sLights: SpotLight[] = [];

    const extractLights = (node: Object3D) => {
      if (node instanceof AmbientLight) aCol = new Color(node.color.r * node.intensity, node.color.g * node.intensity, node.color.b * node.intensity);
    ...
    for (const o of scene.objects) {
      ...
      if (o.material.type === "LambertMaterial") shininess = 0.0;
        ...
        shininess = (o.material as any).shininess || 32;
        specCol = (o.material as any).specularColor ? (o.material as any).specularColor.toArray() : [0,0,0,0];
      }

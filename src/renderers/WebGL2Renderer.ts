...
interface ShaderLocs {
  ...
  color: WebGLUniformLocation | null; specColor: WebGLUniformLocation | null;
  ambient: WebGLUniformLocation | null; dirColor: WebGLUniformLocation | null; dirDir: WebGLUniformLocation | null;
  ...
  texOffset: WebGLUniformLocation | null; // <--- NEU
  texRepeat: WebGLUniformLocation | null; // <--- NEU
}

interface PointLightLocs { pos: WebGLUniformLocation | null; col: WebGLUniformLocation | null; }
interface SpotLightLocs { pos: WebGLUniformLocation | null; dir: WebGLUniformLocation | null; col: WebGLUniformLocation | null; params: WebGLUniformLocation | null; }
...
export class WebGL2Renderer implements IRenderer {
  ...
  public async initialize(canvas: HTMLCanvasElement) {
    ...
    const vs = this.gl.createShader(this.gl.VERTEX_SHADER)!; this.gl.shaderSource(vs, vsCode); this.gl.compileShader(vs);
    const fs = this.gl.createShader(this.gl.FRAGMENT_SHADER)!; this.gl.shaderSource(fs, fsCode); this.gl.compileShader(fs);
  ...
  public render(scene: Scene, vp: Float32Array, camPos: Vector3D = new Vector3D()) {
    ...
    for (const o of scene.objects) {
      ...
      if (this.locs.texOffset) this.gl.uniform2fv(this.locs.texOffset, tOffset); // <--- NEU
      if (this.locs.texRepeat) this.gl.uniform2fv(this.locs.texRepeat, tRepeat); // <--- NEU
  ...
  public setSize(w: number, h: number) {
    this.gl.canvas.width = w * devicePixelRatio; this.gl.canvas.height = h * devicePixelRatio;

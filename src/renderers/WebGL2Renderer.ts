...
export class WebGL2Renderer implements IRenderer {
  ...
  public async initialize(canvas: HTMLCanvasElement) {
    ...
    const vs = this.gl.createShader(this.gl.VERTEX_SHADER)!; this.gl.shaderSource(vs, WireframeVS_300); this.gl.compileShader(vs);
    const fs = this.gl.createShader(this.gl.FRAGMENT_SHADER)!; this.gl.shaderSource(fs, WireframeFS_300); this.gl.compileShader(fs);

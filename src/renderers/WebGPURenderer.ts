...
export class WebGPURenderer extends AbstractRenderer {
  ...
  private _getTexBindGroup(
    ...
    if (m.shaderId === MaterialType.SKYBOX) {
      entries.push({
        binding: 9,
        resource: this._getGPUCubeTextureView(texs["u_skybox"] as CubeTexture),
      });
    } else {
      entries.push({ binding: 2, resource: this._getTextureView(texs["u_diffuseMap"] as Texture) });
      entries.push({
        binding: 3,
        resource: this._getNormalTextureView(texs["u_normalMap"] as Texture),
      });
      entries.push({
        binding: 4,
        resource: this._getTextureView(texs["u_specularMap"] as Texture),
      });

...
export class Matrix4 {
  ...
  public decompose(position: Vector3D, rotation: Vector3D, scale: Vector3D): this {
    ...
    let sx = v1.length(); const sy = v2.length(); const sz = v3.length();
  ...
  public static multiply(a: Matrix4, b: Matrix4, result: Matrix4): void {
    const ae = a.data; const be = b.data; const te = result.data;
  ...
  public static perspective(fov: number, aspect: number, near: number, far: number, target: Matrix4): void {
    const f = 1.0 / Math.tan(fov / 2);
    const rangeInv = 1.0 / (near - far);
    target.data.fill(0);
    target.data[0] = f / aspect; target.data[5] = f;
    target.data[10] = (far + near) * rangeInv; target.data[11] = -1;
  ...
  public static invert(src: Matrix4, target: Matrix4): boolean {
    const te = target.data; const n = src.data;
    const n11 = n[0]!, n12 = n[4]!, n13 = n[8]!, n14 = n[12]!;
    const n21 = n[1]!, n22 = n[5]!, n23 = n[9]!, n24 = n[13]!;
    const n31 = n[2]!, n32 = n[6]!, n33 = n[10]!, n34 = n[14]!;
    const n41 = n[3]!, n42 = n[7]!, n43 = n[11]!, n44 = n[15]!;
    ...
    te[0] = t11 * invDet; te[1] = (n24 * n33 * n41 - n23 * n34 * n41 - n24 * n31 * n43 + n21 * n34 * n43 + n23 * n31 * n44 - n21 * n33 * n44) * invDet;
    ...
    te[4] = t12 * invDet; te[5] = (n13 * n34 * n41 - n14 * n33 * n41 + n14 * n31 * n43 - n11 * n34 * n43 - n13 * n31 * n44 + n11 * n33 * n44) * invDet;
    ...
    te[8] = (n21 * n32 * n44 - n21 * n34 * n42 + n24 * n31 * n42 - n22 * n31 * n44 - n24 * n32 * n41 + n22 * n34 * n41) * invDet;
    te[9] = (n11 * n34 * n42 - n11 * n32 * n44 + n12 * n31 * n44 - n14 * n31 * n42 + n14 * n32 * n41 - n12 * n34 * n41) * invDet;
    te[10] = (n11 * n22 * n44 - n11 * n24 * n42 + n14 * n21 * n42 - n12 * n21 * n44 - n14 * n22 * n41 + n12 * n24 * n41) * invDet;
    te[11] = (n11 * n24 * n32 - n11 * n22 * n34 + n12 * n21 * n34 - n14 * n21 * n32 + n14 * n22 * n31 - n12 * n24 * n31) * invDet;
    te[12] = t14 * invDet; te[13] = (n21 * n33 * n42 - n21 * n32 * n43 + n22 * n31 * n43 - n23 * n31 * n42 + n23 * n32 * n41 - n22 * n33 * n41) * invDet;
    te[14] = (n11 * n32 * n43 - n11 * n33 * n42 + n13 * n31 * n42 - n12 * n31 * n43 - n13 * n32 * n41 + n12 * n33 * n41) * invDet;
    te[15] = (n11 * n23 * n42 - n11 * n22 * n43 + n12 * n21 * n43 - n13 * n21 * n42 + n13 * n22 * n41 - n12 * n23 * n41) * invDet;
    return true;
  }

  public static rotateX(angle: number, target: Matrix4): void {
    const c = Math.cos(angle), s = Math.sin(angle); target.identity();
    target.data[5] = c; target.data[6] = s; target.data[9] = -s; target.data[10] = c;
  }
  public static rotateY(angle: number, target: Matrix4): void {
    const c = Math.cos(angle), s = Math.sin(angle); target.identity();
    target.data[0] = c;  target.data[2] = -s;
    target.data[8] = s;  target.data[10] = c;
  }
  public static rotateZ(angle: number, target: Matrix4): void {
    const c = Math.cos(angle), s = Math.sin(angle); target.identity();
    target.data[0] = c;  target.data[1] = s;
    target.data[4] = -s; target.data[5] = c;
  }
  public static translate(x: number, y: number, z: number, target: Matrix4): void {
    target.identity(); target.data[12] = x; target.data[13] = y; target.data[14] = z;
  }
  public static scale(x: number, y: number | Matrix4, z?: number, target?: Matrix4): void {
    if (y instanceof Matrix4) {
      const m = y; const s = x; m.identity();
      m.data[0] = s; m.data[5] = s; m.data[10] = s; return;
    }
    const m = target!; m.identity();
    m.data[0] = x; m.data[5] = y as number; m.data[10] = z!;
  }
}

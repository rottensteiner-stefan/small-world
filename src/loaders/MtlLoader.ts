...
export class MtlLoader extends Loader<Map<string, PhongMaterial>> {
    ...
    public async load(url: string): Promise<Map<string, PhongMaterial>> {
        ...
        try {
            ...
            this.dispatchEvent(EventType.LOAD_END, { url: fullUrl, data: materials });
            return materials;
        } catch (error) {
    ...
    private parse(text: string): Map<string, PhongMaterial> {
        ...
        for (let line of lines) {
            ...
            if (type === "newmtl") {
                ...
                currentMat.color = new Color(parseFloat(parts[1]), parseFloat(parts[2]), parseFloat(parts[3]));
            } else if (type === "Ks" && currentMat) {
                // Spekuläre Farbe (Glanzlicht)
                currentMat.specularColor = new Color(parseFloat(parts[1]), parseFloat(parts[2]), parseFloat(parts[3]));

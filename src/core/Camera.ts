...
export enum CameraStrategy { FIXED = 0, STIFF = 1, SMOOTH = 2 }
export class Camera {
  ...
  public theta = 0; public phi = 0.6; public radius = 20; public lerpFactor = 0.1;
  ...
  public update(playerPos: Vector3D, dx: number, dy: number) {
    ...
    if (this.strategy !== CameraStrategy.FIXED) {
      this.target.x = playerPos.x; this.target.y = playerPos.y; this.target.z = playerPos.z;
    }
    ...
    if (this.strategy === CameraStrategy.STIFF) {
      this.position.x = idealX; this.position.y = idealY; this.position.z = idealZ;

...
export interface WorldConfig { rendererType?: RendererType | string; canvasId: string; debug?: boolean; worldSize?: number; skyColor?: string; showHUD?: boolean; }
...
export class SmallWorld {
  ...
  public async init(configPath: string): Promise<void> {
    try {
      ...
      if (!this.config.rendererType) { this.config.rendererType = DEFAULT_RENDERER; }
      const canvas = document.getElementById(this.config.canvasId) as HTMLCanvasElement;
      if (!canvas) throw new Error(`Canvas mit ID '${this.config.canvasId}' wurde nicht im DOM gefunden.`);

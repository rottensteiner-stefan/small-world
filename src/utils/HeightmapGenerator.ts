...
export class HeightmapGenerator {
     ...
     */
    public static async generateDiamondSquare(detail: number = 8, roughness: number = 0.6): Promise<ImageBitmap> {
        const size = Math.pow(2, detail) + 1;
        const max = size - 1;
        const map = new Float32Array(size * size);

        // Hilfsfunktionen für das 1D-Array
        const set = (x: number, y: number, val: number) => { map[y * size + x] = val; };

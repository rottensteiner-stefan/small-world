...
export class UniformPacker {
     ...
     */
    static packInto(layout, values, data) {
        data.fill(0);
        if (!layout.uniformLayout) {
            console.error(`[UniformPacker] Material layout is missing 'uniformLayout'. This will result in empty data being sent to the GPU!`);
            return data;
        }
        ...
        for (const name of layout.uniformLayout) {
            ...
            if (!meta) {
                console.warn(`[UniformPacker] Property '${name}' listed in layout but not defined in uniforms.`);
                continue;
            }
...
//# sourceMappingURL=UniformPacker.js.map
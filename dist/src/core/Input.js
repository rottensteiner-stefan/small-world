export class Input {
    ...
    static debug = false;
    static init() {
        window.addEventListener("keydown", e => { this.keys.set(e.code, true); if (this.debug)
            console.log("%c[Input] Key: " + e.code, "color: #ff0"); });
    ...
    static getAxis(neg, pos) {
        let v = 0;
        if (this.keys.get(neg))
            v -= 1;
        if (this.keys.get(pos))
            v += 1;
        return v;
    }
}

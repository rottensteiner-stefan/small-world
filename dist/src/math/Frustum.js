...
export class Frustum {
    ...
    setFromMatrix(m) {
        ...
        for (let i = 0; 6 > i; i++) {
            ...
            const p1 = p[idx + 1];
            const p2 = p[idx + 2];
            const d = Math.sqrt(p0 * p0 + p1 * p1 + p2 * p2);
            if (0 < d) {
                const f = 1.0 / d;
                p[idx] = p[idx] * f;
                p[idx + 1] = p[idx + 1] * f;
    ...
    intersectsBox(box) {
        ...
        for (let i = 0; 6 > i; i++) {
            ...
            const p2 = p[idx + 2];
            const p3 = p[idx + 3];
            const px = 0 <= p0 ? box.max.x : box.min.x;
            const py = 0 <= p1 ? box.max.y : box.min.y;
            const pz = 0 <= p2 ? box.max.z : box.min.z;

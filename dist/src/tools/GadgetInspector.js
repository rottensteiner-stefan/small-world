...
export class GadgetInspector {
     ...
     */
    constructor(_scene, _camera, _canvas, _renderer) {
        this._scene = _scene;
        ...
        const diagFolder = this._pane.addFolder({ title: "Diagnostics", expanded: true });
        diagFolder.addBinding(this._stats, "renderer", { readonly: true, label: "Renderer" });
        this._resolutionBinding = diagFolder.addBinding(this._stats, "resolution", { readonly: true, label: "Resolution" });
        this._fpsBinding = diagFolder.addBinding(this._stats, "fps", { readonly: true, label: "FPS" });
        this._objectsBinding = diagFolder.addBinding(this._stats, "objects", { readonly: true, label: "Total Objects" });
        this._visibleBinding = diagFolder.addBinding(this._stats, "visible", { readonly: true, label: "Visible Objects" });
    ...
    _buildGUI(obj) {
        ...
        if (obj.material) {
            ...
            const bindColor = (propName, label) => {
                if (propName in mat && mat[propName]) {
                    const colorObj = mat[propName];
            ...
            if ("shininess" in mat)
                matFolder.addBinding(mat, "shininess", { min: 0, max: 256 });
            if ("transparent" in mat)
                matFolder.addBinding(mat, "transparent");
            if ("alphaTest" in mat)
                matFolder.addBinding(mat, "alphaTest", { min: 0, max: 1 });
            if ("depthTest" in mat)
                matFolder.addBinding(mat, "depthTest");
            if ("depthWrite" in mat)
                matFolder.addBinding(mat, "depthWrite");
            if ("wireframe" in mat)
                matFolder.addBinding(mat, "wireframe");
            if ("opacity" in mat)
                matFolder.addBinding(mat, "opacity", { min: 0, max: 1 });
            if ("emissiveIntensity" in mat)
                matFolder.addBinding(mat, "emissiveIntensity", { min: 0, max: 10 });
            if ("metalness" in mat)
                matFolder.addBinding(mat, "metalness", { min: 0, max: 1 });
            if ("roughness" in mat)
                matFolder.addBinding(mat, "roughness", { min: 0, max: 1 });
        }
        // Lights
        const maybeLight = obj;
        if ("intensity" in maybeLight && "color" in maybeLight) {
            const lightCol = maybeLight["color"];
        ...
        if (obj.behaviors && obj.behaviors.length > 0) {
            ...
            for (const behavior of obj.behaviors) {
                const behaviorClass = behavior.constructor;
...
//# sourceMappingURL=GadgetInspector.js.map
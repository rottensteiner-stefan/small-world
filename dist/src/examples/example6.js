...
import { AmbientLight, BoundingBox, CameraStrategyType, Capsule, Circle, Color, Cone, Cube, Cylinder, CylinderSector, DirectionalLight, Disk, ExtrudeGeometry, FPSController, Gear, Grid, Object3D, PerspectiveProjection, Plane, Pyramid, Sphere, StandardMaterial, Torus, Triangle, Tube, Vector2D, Vector3D, WireframeMaterial, ZoomController, } from "../index.js";
...
export class Example6 extends AbstractExample {
    ...
    async setupScene() {
        ...
        const getRandomColor = () => {
            return new Color(Math.random() * 0.8 + 0.2, Math.random() * 0.8 + 0.2, Math.random() * 0.8 + 0.2);
        };
        const addGeometryPair = (name, geometry, x, z) => {
            // Wireframe version
            const objWire = new Object3D(`${name}_Wire`);
            ...
            // Solid version
            const objSolid = new Object3D(`${name}_Solid`);
        ...
        const geometries = [
            ...
            {
                name: "Triangle",
                geom: new Triangle(new Vector3D(-1.5, 0, 0), new Vector3D(1.5, 0, 0), new Vector3D(0, 0, -2.6)),
            },
            {
                name: "Gear",

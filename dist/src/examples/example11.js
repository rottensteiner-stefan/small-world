...
import { AmbientLight, BasicMaterial, CameraStrategyType, Color, Cylinder, DeviceCaps, DeviceFeature, Grid, Input, Keys, MathUtils, Object3D, OrbitController, PerspectiveProjection, PointLight, ProjectionType, Sphere, Sprite, SpriteMaterial, Texture, Vector3D, WireframeMaterial, } from "../index.js";
...
class Example11 extends AbstractExample {
    ...
    update(deltaTime) {
        ...
        if (Input.isPressed(Keys.W))
            this.camera.position.z -= moveSpeed; // Forward (-Z)
        if (Input.isPressed(Keys.S))
            this.camera.position.z += moveSpeed; // Backward (+Z)
        if (Input.isPressed(Keys.A))
            this.camera.position.x -= moveSpeed; // Left (-X)
        if (Input.isPressed(Keys.D))
            this.camera.position.x += moveSpeed; // Right (+X)
        if (Input.isPressed(Keys.Q))
            this.camera.position.y += moveSpeed; // Up (+Y)
        if (Input.isPressed(Keys.E))
            this.camera.position.y -= moveSpeed; // Down (-Y)
     ...
     */
    async _addAxis(axis, direction, color) {
        const length = 14;
        const halfLen = length / 2;
        const axisRadius = 0.1;
        const sphereRadius = 0.5;
        // Cylinder (Fat Line)
        const axisLine = new Object3D(`${axis}_Axis`);
        ...
        if ("X" === axis)
            axisLine.rotation.z = -MathUtils.HALF_PI;
        if ("Z" === axis)
            axisLine.rotation.x = MathUtils.HALF_PI;
        this.scene.add(axisLine);
        // End Spheres
        const posPos = new Vector3D().copyFrom(direction).scale(halfLen);
        const negPos = new Vector3D().copyFrom(direction).scale(-halfLen);
    ...
    _addSphere(position, radius, color) {
    ...
    async _addLabel(text, position, color) {
        ...
        try {
            ...
            const sprite = new Sprite(material, `Label_${text}`);

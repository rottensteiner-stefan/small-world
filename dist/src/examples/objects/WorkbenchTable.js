...
const applyBoxUVs = (geo, width, height, depth, offsetX = 0, offsetY = 0, rotateTop = false) => {
    const newGeo = { ...geo }; // Shallow copy
    if (!geo.uvs || geo.uvs.length < 48)
        return newGeo; // Needs at least 24 vertices
...
export class WorkbenchTable extends Object3D {
    constructor(name, options = {}) {
        ...
        for (let i = 0; i < numPlanks; i++) {
            ...
            plank.geometry = applyBoxUVs(unitCubeGeo, w, t, actualPlankDepth, Math.random(), Math.random(), false);
        ...
        const createSideBase = (x, isLeft) => {
            ...
            runnerBody.geometry = applyBoxUVs(unitCubeGeo, runnerWidth, runnerHeight, runnerStraightDepth, Math.random(), Math.random(), true);
            runnerBody.scale.set(runnerWidth, runnerHeight, runnerStraightDepth);
            if (woodMat)
                runnerBody.material = woodMat;
            ...
            if (woodMat)
                runnerFrontCyl.material = woodMat;
            ...
            runnerFrontBox.geometry = applyBoxUVs(unitCubeGeo, runnerWidth, radius, radius, Math.random(), Math.random(), true);
            runnerFrontBox.scale.set(runnerWidth, radius, radius);
            if (woodMat)
                runnerFrontBox.material = woodMat;
            ...
            if (woodMat)
                runnerBackCyl.material = woodMat;
            ...
            runnerBackBox.geometry = applyBoxUVs(unitCubeGeo, runnerWidth, radius, radius, Math.random(), Math.random(), true);
            runnerBackBox.scale.set(runnerWidth, radius, radius);
            if (woodMat)
                runnerBackBox.material = woodMat;
            ...
            legF.geometry = applyBoxUVs(unitCubeGeo, legHeight, legWidth, legDepth, Math.random(), Math.random(), false);
            legF.scale.set(legHeight, legWidth, legDepth);
            legF.rotation.z = Math.PI / 2;
            if (woodMat)
                legF.material = woodMat;
            ...
            legB.geometry = applyBoxUVs(unitCubeGeo, legHeight, legWidth, legDepth, Math.random(), Math.random(), false);
            legB.scale.set(legHeight, legWidth, legDepth);
            legB.rotation.z = Math.PI / 2;
            if (woodMat)
                legB.material = woodMat;
        ...
        const createRail = (z) => {
            ...
            rail.geometry = applyBoxUVs(unitCubeGeo, railLength, railHeight, railWidth, Math.random(), Math.random(), false);
        ...
        for (let i = 0; i < numSlats; i++) {
            ...
            slat.geometry = applyBoxUVs(unitCubeGeo, slatDepth, slatHeight, slatWidth, Math.random(), Math.random(), false);
...
//# sourceMappingURL=WorkbenchTable.js.map
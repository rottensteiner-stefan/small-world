...
import { AmbientLight, CameraStrategyType, Color, Cube, DirectionalLight, FPSController, Gear, GearMath, Object3D, PerspectiveProjection, Plane, Sphere, ProjectionType, PhongMaterial, StandardMaterial, GlassMaterial, Texture, Torus, BoundingBox, Vector2D, Vector3D, SpotLight, FlickerBehavior, ProximitySensorBehavior, Fog, PulsatingBehavior, CullMode, } from "../index.js";
...
class UnderwaterHideoutExample extends AbstractExample {
    _portLight;
    _portLightBehavior;
    _portLightBaseIntensity = 10.0;
    _portLightBaseColor = new Color(1.0, 0.8, 0.5);
    _portMaterial;
    _time = 0;
    _gears = [];
    _waterDrops = [];
    async setupScene() {
        ...
        const vig = this.renderer.postProcessing.get(PostProcessingEffectType.VIGNETTE);
        if (vig) {
            vig.offset = 0.55; // Etwas fetterer Rahmen
            vig.darkness = 1.0; // 100% Schwarz
            vig.roundness = 6.0; // Weiches Rechteck
        }
        const grain = this.renderer.postProcessing.get(PostProcessingEffectType.GRAIN);
        if (grain) {
        ...
        let steamNormal;
        let steamRoughness;
        let steamWallDiffuse;
        let steamWallNormal;
        let steamWallRoughness;
        let crateDiffuse;
        let crateNormal;
        let crateSpecular;
        let brandingDiffuse;
        let brandingNormal;
        let brandingSpecular;
        let rockDiffuse;
        let rockNormal;
        let rockRoughness;
        let portDiffuse;
        ...
        try {
            ...
            steelRoughness = await Texture.fromUrl("/resources/examples/12/scratched_steel_roughness.png");
            if (steelRoughness) {
            ...
            if (crateDiffuse)
                crateDiffuse.repeat.y = -1;
            if (crateNormal)
                crateNormal.repeat.y = -1;
            if (crateSpecular)
                crateSpecular.repeat.y = -1;
            // Crate Branding
            ...
            if (brandingDiffuse)
                brandingDiffuse.repeat.y = -1;
            if (brandingNormal)
                brandingNormal.repeat.y = -1;
            if (brandingSpecular)
                brandingSpecular.repeat.y = -1;
            // Rock Walls (Ends)
        ...
        const createCrate = (name, size, numBrandings = 3) => {
        ...
        const createSpokedGear = (name, module, thickness, numTeeth, numSpokes, rimMaterial, spokeMaterial) => {
        ...
        const gear1Data = createSpokedGear("Gear_Brass_Large", gearModule, 0.4, 16, 5, brassMaterial, steelMaterial);
        ...
        const gear2Data = createSpokedGear("Gear_Steel_Medium", gearModule, 0.3, 10, 4, steelMaterial, brassMaterial);
        ...
        gear2.position.set(-8.8, gear1.position.y + Math.sin(angle1to2) * dist1to2, gear1.position.z + Math.cos(angle1to2) * dist1to2);
        gear2.rotation.y = Math.PI / 2;
        // Mathematically perfect meshing rotation
        const mapToXY = (v) => new Vector3D(v.z, v.y, 0);
        gear2.rotation.z = GearMath.getMeshingRotation(mapToXY(gear1.position), gear1.rotation.z, gear1Data.params, mapToXY(gear2.position), gear2Data.params);
        this.scene.add(gear2);
        this._gears.push({ obj: gear2, speed: GearMath.getDrivenSpeed(0.5, 16, 10) });
        // Gear 3: Steel, small (6 teeth)
        const gear3Data = createSpokedGear("Gear_Steel_Small", gearModule, 0.3, 6, 3, steelMaterial, steelMaterial);
        ...
        gear3.position.set(-8.8, gear2.position.y + Math.sin(angle2to3) * dist2to3, gear2.position.z + Math.cos(angle2to3) * dist2to3);
        ...
        gear3.rotation.z = GearMath.getMeshingRotation(mapToXY(gear2.position), gear2.rotation.z, gear2Data.params, mapToXY(gear3.position), gear3Data.params);
        ...
        const createPorthole = (name, radius) => {
            ...
            glass.addBehavior(new PulsatingBehavior({
                min: 1.0,
                max: 4.0,
                minDuration: 2.0,
                maxDuration: 5.0,
        ...
        this._portLight.addBehavior(new ProximitySensorBehavior({
...
// === START THE ENGINE ===
const app = new UnderwaterHideoutExample();
app
    .start()
    .then(() => {

...
export class GltfLoader extends AbstractLoader {
    ...
    async _parse(gltf, baseUrl) {
        ...
        // 1. Parse Materials
        const materials = await Promise.all((json.materials || []).map((m) => this._parseMaterial(m, json, folderPath)));
        // 2. Create Scene Root
    ...
    _parseGeometry(primitive, json, buffers) {
        ...
        const indices = primitive.indices !== undefined
            ...
            : undefined;
        return new ModelGeometry(Array.from(positions), uvs ? Array.from(uvs) : [], normals ? Array.from(normals) : [], indices ? Array.from(indices) : []).getGeometryData();
    }
    ...
    async _parseMaterial(m, json, folderPath) {

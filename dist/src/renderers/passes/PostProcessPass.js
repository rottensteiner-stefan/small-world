...
export class PostProcessPass {
    ...
    _uniformBuffer;
    _sampler;
    _uniformData = new Float32Array(16);
    _builtTextureView;
    _builtBloomTextureView;
    /**
     * Lazily initialises or rebuilds the pipeline when the HDR texture changes.
     */
    _build(renderer, bloomActiveView) {
    ...
    execute(renderer, _scene, ce, _targetView, _vp, _camPos) {
        const group = renderer.postProcessing;
        if (!group.enabled || !renderer._hdrTextureView)
            return;
        const bloom = group.get(PostProcessingEffectType.BLOOM);
        const bloomActiveView = bloom && bloom.enabled && renderer._bloomTextureView
        ...
        const u32View = new Uint32Array(this._uniformData.buffer, this._uniformData.byteOffset, this._uniformData.length);
...
//# sourceMappingURL=PostProcessPass.js.map
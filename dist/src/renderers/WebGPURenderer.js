...
import { ShaderRegistry } from "../core/index.js";
...
export class WebGPURenderer extends AbstractRenderer {
    ...
    _getPipeline(manifest, topology) {
        ...
        if (!cache) {
            ...
            const targets = [{ format: this._format }];
    ...
    _updateObjUniformBuffer(b, o, m, vMat) {
        ...
        const values = { ...m.properties };
        values["u_model"] = this._scratchModelMatrix;
        if (values["u_color"] === undefined)
            values["u_color"] = o.material?.color.toFloat32Array();
        const packedData = UniformPacker.pack(shaderDef.layout, values);
        this._device.queue.writeBuffer(b, 0, packedData);
    }
    _getTexBindGroup(objBuffer, m, layout) {
        const entries = [
            { binding: 0, resource: { buffer: objBuffer } },
            { binding: 1, resource: this._getSampler(m.textures["u_diffuseMap"]) },
            { binding: 2, resource: this._getTextureView(m.textures["u_diffuseMap"]) },
            { binding: 3, resource: this._getNormalTextureView(m.textures["u_normalMap"]) },
            { binding: 4, resource: this._getTextureView(m.textures["u_specularMap"]) },
            { binding: 5, resource: this._getTextureView(m.textures["u_sandMap"]) },
            { binding: 6, resource: this._getTextureView(m.textures["u_grassMap"]) },
            { binding: 7, resource: this._getTextureView(m.textures["u_rockMap"]) },
            { binding: 8, resource: this._getTextureView(m.textures["u_snowMap"]) },
            {
                binding: 9,
                resource: this._getGPUCubeTextureView(m.textures["u_skybox"]),
            },
        ];
    ...
    _updateGlobalBuffers(vp, camPos, lights) {

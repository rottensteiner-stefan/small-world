...
import { ShaderRegistry, Color, DeviceCaps, DeviceLimit, DepthMaterial, } from "../core/index.js";
import { BlendingMode, CullMode, MaterialType, RendererType, TextureFilter, TextureWrap, Topology, PostProcessingEffectType, } from "../enums/index.js";
...
export class WebGL2Renderer extends AbstractWebGLRenderer {
    ...
    _scratchFloat4 = new Float32Array(4);
    _samplerUnits = {
        u_diffuseMap: 0,
        u_normalMap: 1,
        u_specularMap: 2,
    ...
    _getProgram(shaderId) {
        ...
        if (!cache) {
            ...
            if (!def || !def.sources.glsl300) {
                throw new Error(`[WebGL2Renderer] Shader definition for ${shaderId} not found or missing GLSL 300 source.`);
            }
            ...
            Object.keys(def.layout.uniforms).forEach((name) => {
                ...
                if (null === loc &&
                    ...
                    console.warn(`[WebGL2Renderer] Uniform '${name}' defined in material layout but not found in shader '${shaderId}'. It might be unused or optimized away.`);
                }
    ...
    _getWebGLTexture(tex) {
        ...
        if (!glTex) {
            ...
            const wrapS = TextureWrap.REPEAT === tex.addressModeU
                ? this.gl.REPEAT
                : TextureWrap.MIRRORED_REPEAT === tex.addressModeU
                    ? this.gl.MIRRORED_REPEAT
                    : this.gl.CLAMP_TO_EDGE;
            const wrapT = TextureWrap.REPEAT === tex.addressModeV
                ? this.gl.REPEAT
    ...
    render(scene, vp, camPos = Vector3D.ZERO, vMat) {
        ...
        if (skyboxShaderMap) {
            ...
            for (const [topology, materialGroups] of skyboxShaderMap.entries()) {
                this._renderGroup(MaterialType.SKYBOX, materialGroups, vMat, topology, extractedLights, scene.fog);
            }
        ...
        if (renderList.transparent.length > 0) {
            ...
            for (const obj of renderList.transparent) {
                ...
                this._renderGroup(shaderId, this._scratchTransparentMap, vMat, topology, extractedLights, scene.fog);
            }
        }
        ...
        if (this.postProcessing.enabled && this._hdrFbo && this._postPassGL) {
            ...
            if (this._bloomPassGL) {
                ...
                if (bloomNode && bloomNode.enabled) {
                    bloomTex = this._bloomPassGL.execute(this._hdrFbo.texture, this.gl.canvas.width, this.gl.canvas.height, bloomNode);
                }
            }
     ...
     */
    _renderShadowMaps(lights, sortedGroups) {
        const emptyLights = {
        ...
        for (const light of lights.sLights) {
            ...
            this._updateGlobalUBO(light.shadowCamera.viewProjectionMatrix, light.shadowCamera.position, emptyLights);
     ...
     */
    _renderShadowScene(cache, sortedGroups) {
    ...
    _bindDummyShadowMaps(cache) {
        ...
        if (dirMapLoc)
            this.gl.uniform1i(dirMapLoc, dummyUnit);
        for (let i = 0; i < 4; i++) {
            let loc = cache.uniforms.get(`u_spotShadowMap[${i}]`);
            if (!loc)
                loc = this.gl.getUniformLocation(cache.prog, `u_spotShadowMap[${i}]`) ?? undefined;
            if (loc)
                this.gl.uniform1i(loc, dummyUnit);
        }
    }
    /**
     * Renders a group of objects sharing the same shader and topology.
     */
    _renderGroup(shaderId, materialGroups, vMat, topology = "triangle-list", lights, fog) {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                if (light.castShadow && light.shadowCamera) {
                    ...
                    if (fbo && fbo.texture) {
                        ...
                        const maxUnits = DeviceCaps.getLimit(DeviceLimit.MAX_TEXTURE_IMAGE_UNITS);
                        if (texUnit >= maxUnits) {
                            console.warn(`[WebGL2Renderer] Exceeded MAX_TEXTURE_IMAGE_UNITS (${maxUnits}). Cannot bind spot shadow map to texture unit ${texUnit}.`);
                        }
                        else {
                            this.gl.activeTexture(this.gl.TEXTURE0 + texUnit);
                            this.gl.bindTexture(this.gl.TEXTURE_2D, fbo.texture);
                            if (mapLoc)
                                this.gl.uniform1i(mapLoc, texUnit);
                            if (matLoc)
        ...
        if (lights && lights.dLight && lights.dLight.castShadow && lights.dLight.numCascades > 0) {
            ...
            if (fbo && fbo.texture) {
                ...
                const maxUnits = DeviceCaps.getLimit(DeviceLimit.MAX_TEXTURE_IMAGE_UNITS);
                if (texUnit >= maxUnits) {
                    console.warn(`[WebGL2Renderer] Exceeded MAX_TEXTURE_IMAGE_UNITS (${maxUnits}). Cannot bind directional shadow map to texture unit ${texUnit}.`);
                }
                else {
                    this.gl.activeTexture(this.gl.TEXTURE0 + texUnit);
                    this.gl.bindTexture(this.gl.TEXTURE_2D, fbo.texture);
                    ...
                    for (let i = 0; i < light.numCascades; i++) {
                        ...
                        if (matLoc && light.cascadeCameras[i]) {
                            this.gl.uniformMatrix4fv(matLoc, false, light.cascadeCameras[i].viewProjectionMatrix);
                        }
                    }
                    if (splitsLoc) {
            ...
            if (shaderId === MaterialType.SKYBOX) {
                ...
                for (const uniformName in this._samplerUnits) {
                    ...
                    if (loc) {
                        const maxUnits = DeviceCaps.getLimit(DeviceLimit.MAX_TEXTURE_IMAGE_UNITS);
                        if (unit >= maxUnits) {
                            console.warn(`[WebGL2Renderer] Exceeded MAX_TEXTURE_IMAGE_UNITS (${maxUnits}). Cannot bind material texture ${uniformName} to unit ${unit}.`);
                        }
                        else {
    ...
    _updateGlobalUBO(vp, camPos, lights) {

...
export class PostProcessPassGL {
    ...
    _uExposure = null;
    _uGamma = null;
    _uToneMappingMode = null;
    _uVignetteEnabled = null;
    _uVignetteOffset = null;
    _uVignetteDarkness = null;
    _uVignetteRoundness = null;
    _uGrainEnabled = null;
    _uGrainIntensity = null;
    _uTime = null;
    // Bloom
    _uBloomTexture = null;
    _uBloomEnabled = null;
    _uBloomIntensity = null;
    _uBloomColor = null;
    _aPos = -1;
    _isWebGL2;
     ...
     */
    execute(gl, hdrTexture, group, bloomTexture = null) {
        if (!this._prog)
            return;
        ...
        // Bloom
        if (bloomTexture &&
            group.get(PostProcessingEffectType.BLOOM)
                ?.enabled) {
            const bloom = group.get(PostProcessingEffectType.BLOOM);
        ...
        const tm = group.get(PostProcessingEffectType.TONE_MAPPING);
        const vig = group.get(PostProcessingEffectType.VIGNETTE);
        const grain = group.get(PostProcessingEffectType.GRAIN);
        if (this._uHdrTexture !== null)
            gl.uniform1i(this._uHdrTexture, 0);
        if (this._uExposure !== null)
            gl.uniform1f(this._uExposure, tm && tm.enabled ? tm.exposure : 1.0);
        if (this._uGamma !== null)
            gl.uniform1f(this._uGamma, tm && tm.enabled ? 1.0 / tm.gamma : 1.0);
        ...
        if (vig && vig.enabled) {
            if (this._uVignetteOffset)
                gl.uniform1f(this._uVignetteOffset, vig.offset);
            if (this._uVignetteDarkness)
                gl.uniform1f(this._uVignetteDarkness, vig.darkness);
            if (this._uVignetteRoundness)
                gl.uniform1f(this._uVignetteRoundness, vig.roundness);
        }
        else {
            if (this._uVignetteEnabled)
                gl.uniform1i(this._uVignetteEnabled, 0);
        }
        if (this._uGrainEnabled !== null)
            gl.uniform1i(this._uGrainEnabled, grain && grain.enabled ? 1 : 0);
        if (grain && grain.enabled) {
            if (this._uGrainIntensity)
                gl.uniform1f(this._uGrainIntensity, grain.intensity);
        }
        if (this._uTime !== null)
...
//# sourceMappingURL=PostProcessPassGL.js.map
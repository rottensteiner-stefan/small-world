/// src/renderers/post/BloomPassGPU.ts
...
export class BloomPassGPU {
    ...
    _downBindGroups = [];
    _upBindGroups = [];
    _uniformBuffers = []; // Reuse buffers across passes
    _width = 0;
    _height = 0;
    _mipCount = 5;
    constructor(device) {
    ...
    _resizeMipChain(width, height, sourceView) {
        ...
        if (this._bloomTexture) {
            ...
            for (const buf of this._uniformBuffers)
                buf.destroy();
        ...
        for (let i = 0; i < totalPasses; i++) {
            this._uniformBuffers.push(this._device.createBuffer({
        ...
        for (let i = 0; i < this._mipCount; i++) {
            this._mipViews.push(this._bloomTexture.createView({
                baseMipLevel: i,
                mipLevelCount: 1,
            }));
        }
        ...
        for (let i = 0; i < this._mipCount; i++) {
            ...
            this._downBindGroups.push(this._device.createBindGroup({
                layout: bgl,
                entries: [
        ...
        for (let i = this._mipCount - 2; i >= 0; i--) {
            ...
            this._upBindGroups.push(this._device.createBindGroup({
                layout: bgl,
                entries: [
                    { binding: 0, resource: this._sampler },
                    { binding: 1, resource: readView },
    ...
    execute(ce, hdrTexture, hdrTextureView, bloomConfig) {
...
//# sourceMappingURL=BloomPassGPU.js.map
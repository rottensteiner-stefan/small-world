import { BoundingType } from "../interfaces/IBoundingVolume.js";
import { Vector3D } from "../math/Vector3D.js";
export class Collision {
    static test(a, b) {
        // --- PHASE 1: Broad Phase (Sphere vs Sphere) ---
        const distSq = a.center.distanceToSq(b.center);
        const sumRad = a.getBroadRadius() + b.getBroadRadius();
        if (distSq > sumRad * sumRad)
            return false;
        // --- PHASE 2: Narrow Phase ---
        if (a.type === BoundingType.SPHERE && b.type === BoundingType.SPHERE) {
            return this.sphereSphere(a, b);
        }
        if (a.type === BoundingType.BOX && b.type === BoundingType.BOX) {
            return this.boxBox(a, b);
        }
        // Gemischte Typen (Sphere vs Box)
        if (a.type === BoundingType.SPHERE && b.type === BoundingType.BOX) {
            return this.sphereBox(a, b);
        }
        if (a.type === BoundingType.BOX && b.type === BoundingType.SPHERE) {
            return this.sphereBox(b, a);
        }
        return false;
    }
    static sphereSphere(s1, s2) {
        const d2 = s1.center.distanceToSq(s2.center);
        const r2 = (s1.radius + s2.radius) * (s1.radius + s2.radius);
        return d2 <= r2;
    }
    static boxBox(b1, b2) {
        return (b1.min.x <= b2.max.x &&
            b1.max.x >= b2.min.x &&
            b1.min.y <= b2.max.y &&
            b1.max.y >= b2.min.y &&
            b1.min.z <= b2.max.z &&
            b1.max.z >= b2.min.z);
    }
    static sphereBox(s, b) {
        // Clamping Trick: Finde den nächsten Punkt auf der Box zur Kugel
        const closest = new Vector3D(Math.max(b.min.x, Math.min(s.center.x, b.max.x)), Math.max(b.min.y, Math.min(s.center.y, b.max.y)), Math.max(b.min.z, Math.min(s.center.z, b.max.z)));
        return closest.distanceToSq(s.center) <= s.radius * s.radius;
    }
}
//# sourceMappingURL=Collision.js.map
  ...
  protected override update(deltaTime: number): void {
    ...
    for (let m: number = 0; m < this._lavaPlanes.length; m++) {
      ...
      for (let i: number = 0; i < vertices.length; i += 3) {
        ...
        const n1 =
          this._noise(vx * 0.4 + this._time * 0.1725, vz * 0.4 + this._time * 0.115 + seed) *
          0.2875;
        // 2. Layer: Faster, smaller bubbles/ripples (Speed and Amplitude +15%)
        const n2 =
          this._noise(vx * 1.5 - this._time * 0.345, vz * 1.5 + this._time * 0.23 + seed) * 0.092;
    ...
    for (let l: number = 0; l < this._lavaLights.length; l++) {
      ...
      const pulse =
        (Math.sin(this._time * 1.5 + seed) * 0.5 + 0.5) * 0.4 +

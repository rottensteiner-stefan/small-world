...
class Example17 extends AbstractExample {
    ...
    for (let z = 0; z <= sizeZ; z++) {
      for (let x = 0; x <= sizeX; x++) {
        ...
        for (let octave = 1; octave <= 16; octave++) {
          ...
          // Extra useless iterations to make it artificially heavy for demonstration
          for (let k = 0; k < 50; k++) {
  ...
  private async _generate(useWorker: boolean): Promise<void> {
    ...
    try {
      if (useWorker) {
        ...
        result = await this._threadPool.execute(Example17._generateTerrainChunk, terrainSettings);
      } else {
        // Block main thread!
        result = Example17._generateTerrainChunk(terrainSettings);
      }

...
class Showcase11 extends AbstractShowcase {
  ...
  private async _addLabel(text: string, position: Vector3D, color: Color): Promise<void> {
    ...
    try {
      ...
      console.log(`[Showcase 11] Label added: ${text} at`, position);
    } catch (e) {
      console.error(`[Showcase 11] Error creating label texture for ${text}:`, e);
    }
  }
}

const app: Showcase11 = new Showcase11();
app.start().catch((error: unknown) => {
  console.error("[Showcase 11] Error starting application:", error);
});

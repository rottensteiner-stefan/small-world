...
class Showcase16 extends AbstractShowcase {
  ...
  private _runOnMainThread(): void {
    ...
    const result = Showcase16._heavyMathLogic({ limit: 5000000 });
  ...
  private async _runOnWorkerThread(): Promise<void> {
    ...
    try {
      const result = await this._threadPool.execute(Showcase16._heavyMathLogic, { limit: 5000000 });
      const elapsed = performance.now() - startTime;
      this._setUiResult(

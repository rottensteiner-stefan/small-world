...
**Small World** is not trying to be the next Three.js or Babylon.js. It is trying to be the **"Preact of 3D Engines."**
...
**The Problem:** Big engines are monolithic. A simple Babylon.js scene can easily exceed 1.5MB of code. Three.js, despite its modularity, struggles with effective tree-shaking for tiny applications. For Playable Ads (which have strict 2MB constraints including assets), Telegram Mini-Apps, edge computing, and fast Web3 frontends, loading times are critical.
...
**The Problem:** Three.js is a rendering library, not an engine. It provides no standard architecture for game loops, state management, or input handling. Developers either reinvent the wheel or adopt heavy wrappers like React Three Fiber, dragging the massive overhead of React's reconciliation loop into the 60FPS 3D world.
**Our Solution:** The integrated **Behavior System** (`addBehavior(new HoverBehavior())`) and **Interaction Manager** (`onPointerClick`). Small World provides out-of-the-box, drag-and-drop game logic and strict Finite State Machines (`StateMachine`) that mimic the developer experience of Unity or Unreal, without the monolithic framework tax.

### 3. Developer Experience (DX) & TypeScript
**The Problem:** Legacy engines suffer from years of JavaScript cruft. Their TypeScript definitions (`@types/three`) are often asynchronous to the actual codebase, patched together with `any` types that silently fail at runtime.
...
2. **DX is King:** Built-in tools like the `GadgetInspector` and the `IBL Generator` ship with the engine because developers shouldn't have to spend a day configuring `lil-gui` just to tweak a light's intensity.
